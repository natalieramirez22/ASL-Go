...
class CameraViewController: RecorderViewController {
    ...
    private let label = UILabel(frame: CGRect(x: 100, y: 100, width: 300, height: 100))
    //private let hint = UIImageView(frame: CGRect(x: 400, y: 100, width: 200, height: 200))
    ...
    private func handleGestureStateChange(state: ApprovalGestureProcessor.State) {
        ...
        case .possibleThumbsDown:
            tipsColor = .orange
            label.text = ""
        case .cat, .possiblecat:
            tipsColor = .green
            label.text = "cat"
        case .thumbsDown, .unknown:
            tipsColor = .red
            label.text = " "
        case .iloveasl, .possibleilove:
            tipsColor = .green
            label.text = "I love ASL!"
        case .hello, .possiblehello:
            tipsColor = .green
            label.text = "Hello!"
        case .noo, .possibleno:
            tipsColor = .green
